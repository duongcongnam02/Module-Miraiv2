...
module.exports.config = {
    ...
    cooldowns: 0,
    envConfig: {
        key: "mzk_DzqTe9" // lấy key tại: manhict.tech. Nhớ dán key vào trong config nha
    },
    dependencies: {
        "image-downloader": ""
    }
...
module.exports.run = async ({
    ...
    try {
        ...
        let os = (await axios.get(encodeURI(`http://manhict.tech/tiktok/search?query=${search}&apikey=${key || envConfig.key}`))).data
        for (const i in os.results) {
            msg += `${++count}. ${os.results[i].desc}\n - Thời lượng: ${C(os.results[i].time)}\n\n`
            arr.push({
                i: count,
                n: os.results[i].desc.toString(),
                t: os.results[i].time,
                v: os.results[i].video.toString()
            })
            if (count == 10) break
        }
